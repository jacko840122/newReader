package com.common.kuaxue.app.bookstore;
...
import com.common.kuaxue.ebookstore.core.NetRequest;
import com.greenlemonmobile.app.ebook.MyMd5;
import com.common.kuaxue.app.bookstore.BookCacheData.DownloadBookInfo;
...
public class DownLoadService extends Service {
	...
	private ArrayList<BooksDBOnInternet.BookInfo> mDowndLoadBooksCoverList;
...
//		mEbookDownloadList=new ArrayList<DownLoadService.EbookDownload>();
	...
	private boolean IsInDownloadList(String book_id){
		synchronized (mDowndLoadBooksCoverList) {
			...
			for(int i=0,count=mDowndLoadBooksCoverList.size();i<count;i++){
				BooksDBOnInternet.BookInfo bookInfo=mDowndLoadBooksCoverList.get(i);
	...
	public void onStart(Intent intent, int startId) {
		...
		if(ACTION_DOANLOAD_EBOOK.equals(action)||ACTION_PAUSE_DOANLOAD_EBOOK.equals(action)
			...
			BooksDBOnInternet.BookInfo mBookInfo = (BooksDBOnInternet.BookInfo)intent.getSerializableExtra("BookInfo");
			if (mBookInfo != null) {
			...
			final ArrayList<BooksDBOnInternet.BookInfo> bookinfoS=intent.getParcelableArrayListExtra("BookInfos");
			...
			new Thread(new Runnable() {
				...
				public void run() {				
					...
					for(BooksDBOnInternet.BookInfo bookinfo:bookinfoS){
	...
	private void DownLoadCover(BooksDBOnInternet.BookInfo bookinfo){
		...
		try {
			...
			final BooksDBOnInternet.BookInfo bookinfo2=bookinfo;
	...
	private void addDownLoad(BooksDBOnInternet.BookInfo bookInfo){
		synchronized (mLock) {
			for (EbookDownload mEbookDownload:mEbookDownloadList) {
	...
	private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case MSG_PAUSE:{
				BooksDBOnInternet.BookInfo mBookInfo = (BooksDBOnInternet.BookInfo)msg.obj;
			...
			case MSG_ADDDOWN:
				addDownLoad((BooksDBOnInternet.BookInfo)msg.obj);
				updateDownLoad();
				break;
